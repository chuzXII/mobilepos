  ...
  const formd = () => {
    return (
      <View style={styles.container}>
        <Text style={{ color: '#000', fontSize: 28, marginBottom: 40,fontFamily:'TitilliumWeb-Bold' }}>Setup Spredsheet</Text>
        <View style={styles.card}>
          ...
          <View style={styles.warpcard}>
            ...
            <TouchableOpacity style={[styles.buttonTutorial,{marginBottom:23}]} onPress={() => onPressTutorial("BD")}>
    ...
    const get= async()=>{
       const cek = await AsyncStorage.getItem('TokenSheetFull')
       setCek(cek)
       setForm(cek)
    }
  useEffect(()=>{
